...
//题2 删除为x的值
bool List_DelCall(LinkList &L,int x){
    LNode *q,*p=L;
    //下一个的判空,空了就没有继续递归的必要了
    if(p->next==NULL) return true;
    //继承指针地址,这里是下一个节点的地址
    q=p->next;
    //验证，如果不符合就下一个，说明下一个节点不符合要求才回归
    if(q->data != x)  return List_DelCall(p->next,x);
    //符合验证，删除后，下一个
    //更改继承
    p->next=q->next;
    //释放
    free(q);
    return List_DelCall(p,x);

}
//循环大法 
bool List_DelLoop(LinkList &L ,int x){
    if(L->next == NULL) return false;//判空
    LNode *p=L,*q= p->next;//保存
    while(q->next !=NULL ){//判断空
        if(q->data !=x ){//验证
            p=q;  //切换
        }else {
        p->next=q->next;//继承
...
//题3 逆向输出
int List_ShowCall(LinkList &L){
    if(L->next != NULL)
        printf("%d   ",List_ShowCall(L->next));
    return L->data;
}
