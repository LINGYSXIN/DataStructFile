...
//代码优化，把后插代码封装起来
bool NodeInserTail(LNode *p,int e){
...
/*
题目：
1.设计递归算法，删除不带头节点的单链表中所有值为x的节点


*/
//第1题
//从第一个节点开始删
bool List_DelCall(LinkList &L,int e){
    if(L==NULL)         //判断该节点是否合法，如果p是L，正好判断是不是空链表
        return true;   //这个节点空了，
    //说明不是空节点,不为空就有next
    //查看该节点是否符合删除条件
    if(L->data !=e){ //说明该节点值不为e
        return List_DelCall(L->next,e);//下一个节点
    }
    //说明该节点符合删除条件
    LNode *q=L;   //保存该节点指针
    L=L->next;    //将后继的后继放入旧节点
    free(q);      //释放节点内存
    return List_DelCall(L,e);//下一个节点
}
